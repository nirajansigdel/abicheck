    ...
    try {
        ...
        if (!password || !passwordRegex.test(password)) {
            return res.status(400).json({
                message: "Password should contain at least one digit, one uppercase letter, and one special character.",
            });
        }
        ...
        const sql = 'INSERT INTO authentication (email, password, role) VALUES (?, ?, ?)';
        databaseconnector.connection.query(sql, [email, password, userRole], (error, result) => {
            if (error) {
                console.error(error);
